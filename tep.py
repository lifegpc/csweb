...
def addWikiLinkToText(s: str, key: str, itemName: str, itemId: str, display: str):  # noqa: E501
    '''添加维基链接
    s 原始字符串：如<salt>:
    key 要替换的键：salt
    itemName 维基百科内容标题
    ...
    display 显示名称'''
    return s.replace(f'<{key}>', f'<a class="wplink" item="{itemName}" qid="{itemId}">{display}</a>')  # noqa: E501
