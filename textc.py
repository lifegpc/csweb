# (C) 2021 lifegpc
# This file is part of rssbot. (Copy from rssbot)
#
...
class TGHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs):
        ...
        t = {'type': typ, 'offset': len(self.s), 'length': 0, 'tag': tag,
             'start': self.getpos()[1], 'end': 0, 'datastart': -1}
    ...
    def handle_endtag(self, tag: str):
        if len(self.__entryL) > 0:
            if self.__entryL[-1]['datastart'] == -1:
                self.__entryL[-1]['datastart'] = self.getpos()[1]
        if tag not in ['b', 'strong', 'i', 'em', 'u', 'ins', 's', 'strike',
                       'del', 'a', 'pre', 'code']:
            return
...
class MessageEntries:
    ...
    def getStr(self, l: int, s: str, re: str) -> (str, str):  # noqa: E741
        """split string
        s is origin string
        re is parsed string"""
        s = s.replace(
    ...
    def isOkWithOrigin(self, l: int) -> bool:  # noqa: E741
        for v in self.__list:
            if l > v['start'] and l < v['end']:
                return False
        return True

    def isOkWithRe(self, l: int) -> bool:  # noqa: E741
        for v in self.__list:
...
class textc:
    ...
    def cut(self):
        ...
        l = self.__str.splitlines(True)  # noqa: E741
...
def removeEmptyLine(s: str) -> str:
    l = s.splitlines(False)  # noqa: E741
    r = []
    for v in l:
        if v != '':
            r.append(v)
